...
export function useGameTimer(options: UseGameTimerOptions = {}): UseGameTimerReturn {
    ...
    setTimeRemaining(prev => {
      ...
      if (newTime <= 0 && isActive) {
        ...
        if (autoSubmitOnExpiry && drawingContext && !drawingContext.hasSubmitted) {
          console.log('useGameTimer: Timer expired, triggering auto-submit', {
            autoSubmitOnExpiry,
          ...
          console.log('useGameTimer: Timer expired but auto-submit not triggered', {
            autoSubmitOnExpiry,
  ...
  const handleTimerSync = useCallback((event: any) => {
  ...
  useEffect(() => {
    if (currentGame && drawingContext && currentGame.status === 'drawing' && !isActive && !drawingContext.hasSubmitted) {
      console.log('useGameTimer: Starting timer with:', {
      ...
      console.log('useGameTimer: Not starting timer - user has already submitted');
    }
