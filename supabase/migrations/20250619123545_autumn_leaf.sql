...
CREATE TABLE IF NOT EXISTS votes (
  ...
  -- Ensure voter is participant and not voting for themselves
  CONSTRAINT no_self_voting CHECK (
    voter_id != (SELECT user_id FROM submissions WHERE id = submission_id)
  )
);
...
-- Booster packs for premium content
...
CREATE POLICY "Users can update their own submissions before voting" ON submissions
  FOR UPDATE USING (
    ...
    game_id IN (
      ...
      WHERE status IN ('waiting', 'briefing', 'drawing')
    )
  );
...
CREATE OR REPLACE TRIGGER trigger_update_vote_count
...
-- Grant free packs to all existing users
